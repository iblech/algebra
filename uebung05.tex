...
\item[2.] Die Gleichung~$X^3 - 1 = 0$ hat~$p = 0$, $q = -1$ und die drei
...
\begin{align*}
  \sum_{k=0}^\infty \frac{f^{(k)}(x)}{k!} (X-x)^k &=
  \sum_{k=0}^\infty \frac{(X^n)^{(k)}(x)}{k!} (X-x)^k \\
  &= \sum_{k=0}^n \frac{n! / (n-k)! \cdot x^{n-k}}{k!} (X-x)^k \\
  &= \sum_{k=0}^n \binom{n}{k} x^{n-k} (X-x)^k \\
  &= (x + (X-x))^n = X^n = f.
\end{align*}
\end{loesungE}
\end{aufgabe}
