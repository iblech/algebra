...
\begin{align*}
  ...
  &\leq n R^{i-1} \cdot |z-w| \leq n R^{n-1} \cdot |z-w|
...
letzte Abschätzung~$R^{i-1} \leq R^{n-1}$ verzichten. Dann kann man~$\delta$
sogar etwas kleiner wählen (das ist besser), nämlich als
\[ \delta = \epsilon \cdot \left[
  n \cdot \sum_{i=0}^n |a_i| R^{i-1} + 1
\right]^{-1}, \]
...
\begin{align*}
  |f(z) - f(w)| &=
    ...
    \sum_{i=0}^n |a_i| |z^i - w^i| \\
  &\leq
    \sum_{i=0}^n |a_i| \cdot n R^{i-1} \cdot |z-w| =
    \left(n \cdot \sum_{i=0}^n |a_i| R^{i-1}\right) \cdot |z-w| < \epsilon.
\end{align*}
